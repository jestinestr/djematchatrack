...
router.patch('/:id/fee', async (req, res) => {
  const { fee_per_gram, fee_currency, fine_amount, unboxing_fee, unit_fee } = req.body;
  ...
  if (unit_fee !== undefined && !isNaN(Number(unit_fee))) {
    updates.unit_fee = Math.max(0, Number(unit_fee));
  }
